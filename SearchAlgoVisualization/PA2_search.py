...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        for neighbor in get_neighbors(curNode):
            pathCostofNeighbor = move_cost(neighbor, barrier)
            if neighbor not in fringe and neighbor not in expandedNodes:
               fringe.add(neighbor)
            if (neighbor not in G and neighbor not in expandedNodes) or\
                    (pathCostofNeighbor + G[curNode] < G[neighbor] and neighbor not in expandedNodes):
                parent[neighbor] = curNode
                G[neighbor] = pathCostofNeighbor + G[curNode]
        ###########################################################################################
        ##                                   END OF YOUR CODE                                    ##
        ###########################################################################################
    ...
    while len(fringe) > 0:
        ...
        for neighbor in neighbors:
            if neighbor not in expandedNodes:
                ...
                elif F[neighbor] > G[curNode] + move_cost(neighbor, barrier) + heuristic(neighbor, goal):
                    parent[neighbor] = curNode
                    G[neighbor] = G[curNode] + move_cost(neighbor, barrier)
                    F[neighbor] = G[neighbor] + heuristic(neighbor, goal)
